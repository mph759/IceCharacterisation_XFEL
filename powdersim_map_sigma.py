...
nsigma = 10000
sigmasamp = np.arange(nsigma)*(sigma_max-sigma_min)/nsigma + sigma_min
pp = [191,216,245,378,489]
pplabels = ["100", "002", "101","102","110"]
peakvsig = np.zeros( (nsigma,len(pp)))

for isig, sigma in enumerate(sigmasamp):
    print( "sigma", isig, end="\r")
    po_norm   = 2*sigma*erf(1/(2*sigma))
    ncsamp = 1000 
    ...
    i#for i in range(5): print(f'({pp[i]})'+"  q = ", x[pp[i]], " ; I = ", npowder[pp[i]])

    for i in range(5): peakvsig[isig,i] = npowder[pp[i]]


p,  = plt.plot( sigmasamp, peakvsig[:,1])
p2, = plt.plot( sigmasamp, peakvsig[:,2])
plt.xlabel( "sigma" )
plt.ylabel( "Peak ratio")
plt.legend([p,p2], [pplabels[1], pplabels[2]])

np.save( outpath+tag+"_sigma.npy", sigmasamp)
np.save( outpath+tag+"_peakratios.npy", peakvsig)
plt.draw()
plt.show()
